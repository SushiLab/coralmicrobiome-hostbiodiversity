...
## Script purpose: Compute similarity across treatment and source by host
## Date: April 2024
...
for (k in levels(metadat$source)){
  for (i in levels(metadat$treatment_binary)){
    ...
    # Write file
    edge_df_sim<-1-edge_df #convert to similarity
    ed<-edge_df_sim %>%
      rownames_to_column("interaction") %>%
      separate(interaction,into=c("from","to"),sep=";") %>%
      rename(similarity=dist)
    fwrite(ed,paste("../data/24_BDiv_ed_",k,i,".tsv",sep=""),sep="\t",na="NA")
    ...
    tmp<-asv_bctab_red %>%
      ...
      dplyr::select(bc_dissim,comparison,sample_1_treatment_binary) %>%
  ...
  # Compare treatments
  edge_stats<-compare_means(bc_dissim~treatment_binary,data=stats_dist,group.by="comparison",p.adjust.method="holm") %>% as.data.frame()
  fwrite(edge_stats,paste("../data/24_BDiv_stats_ed_",k,".tsv",sep=""),sep="\t",na="NA")
} 
