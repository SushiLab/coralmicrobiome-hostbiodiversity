...
asv_dat<-fread("../data/asv_dat_original_host-associated.tsv",sep="\t",header=TRUE,data.table=FALSE)
...
is_negative<-data.frame(id=rownames(asv_abtab)) %>%
  ...
  mutate(is_neg=ifelse(sample %in% c("PBS control","PCR blank","extraction blank","Tris-NaCl control"),TRUE,FALSE)) %>%
  pull(is_neg)
...
mkdir("../data/decontam_host-associated")
for (thresh in c(0.001,0.002,0.005,0.01,0.02,0.05,0.1,0.2,0.5)){
  ...
  fwrite(res,file=paste("../data/decontam_host-associated/decontam_stats_thresh-",thresh,".tsv",sep=""),sep="\t")
  fwrite(asv_dat_decontam,file=paste("../data/decontam_host-associated/asv_dat_decontam_thresh-",thresh,".tsv",sep=""),sep="\t")
  cat("\n")
}

# Explore percentage of original reads/ASVs kept after decontam using various thresholds
# Load decontaminated ASV files
asv_dat_paths<-list.files("../data/decontam_host-associated/",pattern="asv_dat_decontam_thresh",full.names=TRUE)
...
ggplot(data=df_plot_species,aes(x=file,y=perc_original_reads_median,ymin=perc_original_reads_25CI,ymax=perc_original_reads_75CI,fill=species,colour=species)) +
  ...
  theme_bw()
ggsave(filename="../data/03_perc-reads-kept-by-species.png",width=10,height=7,dpi=500)
...
ggplot(data=df_plot_species,aes(x=file,y=perc_original_spp_median,ymin=perc_original_spp_25CI,ymax=perc_original_spp_75CI,fill=species,colour=species)) +
  ...
  guides(colour="none",fill="none") +
  theme_bw()
ggsave(filename="../data/03_perc-asvs-kept-by-species.png",width=10,height=7,dpi=500)
