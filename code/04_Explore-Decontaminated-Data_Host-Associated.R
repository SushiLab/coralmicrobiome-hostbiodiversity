...
asv_abtab_decontam<-fread("../data/decontam_host-associated/asv_dat_decontam_thresh-0.1.tsv",sep="\t",header=TRUE,data.table=FALSE) %>%
...
asv_abtab_nodecontam_rr<-asv_abtab[which(apply(asv_abtab,1,sum)>=rr_cutoff),] #only keep samples >=100 reads
asv_abtab_nodecontam_rr<-rrarefy(asv_abtab_nodecontam_rr,sample=rr_cutoff) #subsample to 100 reads
...
bc_dist_decontam<-vegdist(sqrt(asv_abtab_decontam_rr)) %>% as.matrix() %>% as.data.frame() %>%
  ...
  left_join(select(metadat,id,sample),by=c("id_2"="id")) %>%
  filter(sample.x %in% c("mucus","biofilm","tissue") & sample.y %in% c("PBS control","PCR blank","extraction blank","Tris-NaCl control")) %>%
  mutate(sample.x=factor(sample.x,levels=c("mucus","biofilm","tissue")))
...
ggsave(filename="../data/04_sample-to-blank-original-vs-cleaned.png",width=10,height=7,dpi=500)
...
ggsave(filename="../data/04_sample-to-blank-original-vs-cleaned-by-species.png",width=10,height=7,dpi=500)
...
ggplot(data=pcoa_df,aes(x=Axis.1,y=Axis.2,colour=species)) +
  ...
  facet_wrap(~sample) +
  theme_bw()
ggsave(filename="../data/04_PCoA_by-species.png",width=10,height=7,dpi=500)
...
ggsave(filename="../data/04_PCoA_by-organismal-group.png",width=unit(12,"cm"),height=unit(5,"cm"))
