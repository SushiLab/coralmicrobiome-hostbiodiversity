...
# Load data
asv_abtab<-fread("../data/asv_abtab_original_host-associated.tsv",sep="\t",header=TRUE,data.table=FALSE) %>%
  column_to_rownames("id") %>% as.matrix()
metadat<-fread("../data/metadata_host-associated.csv",header=TRUE,data.table=FALSE) %>%
  mutate(sample=factor(sample,levels=c("mucus","biofilm","tissue",
...
ggsave(filename="../data/02_seq-depth.png",width=10,height=7,dpi=500)
...
ggplot(data=pcoa_df,aes(x=Axis.1,y=Axis.2,colour=sample)) +
  ...
  guides(colour="none") +
  theme_bw()
ggsave(filename="../data/02_PCoA_by-sample-type.png",width=10,height=7,dpi=500)
...
ggplot(data=pcoa_df,aes(x=Axis.1,y=Axis.2,colour=sample)) +
  ...
  facet_wrap(~species) +
  theme_bw()
ggsave(filename="../data/02_PCoA_by-species.png",width=10,height=7,dpi=500)

# Analyse sample-to-blank distances
bc_dist<-vegdist(sqrt(asv_abtab_rr)) %>% as.matrix() %>% as.data.frame() %>%
  rownames_to_column("id_1") %>%
  pivot_longer(-id_1,names_to="id_2",values_to="distance") %>%
  left_join(select(metadat,id,sample,aDNA_smear_conc_nmol_l),by=c("id_1"="id")) %>%
  left_join(select(metadat,id,sample),by=c("id_2"="id")) %>%
  filter(sample.x %in% c("mucus","biofilm","tissue") & sample.y %in% c("PBS control","PCR blank","extraction blank","Tris-NaCl control"))

# Explore clustering of mucus vs controls further
metadat_rr<-data.frame(id=rownames(asv_abtab_rr)) %>%
  left_join(metadat,by="id")
muc<-which(metadat_rr$sample %in% c("mucus","PCR blank","extraction blank"))
asv_abtab_rr_muc<-asv_abtab_rr[muc,]

# Compare mucus vs control communities
pcoa_muc<-pcoa(D=vegdist(sqrt(asv_abtab_rr_muc)))
pcoa_df<-pcoa_muc$vectors %>% as.data.frame() %>%
  rownames_to_column("id") %>%
  left_join(metadat,by="id") %>%
  mutate(sample=factor(sample,levels=c("mucus","extraction blank","PCR blank")))

# Plot mucus vs control PCoA
ggplot(data=pcoa_df,aes(x=Axis.1,y=Axis.2,colour=sample)) +
  geom_hline(yintercept = 0,linetype=2) +
  geom_vline(xintercept=0,linetype=2) +
  geom_point(size=4,alpha=0.5) +
  scale_colour_manual(values=colours) +
  theme_bw()
ggsave(filename="../data/02_PCoA_mucus-vs-controls.png",width=10,height=7,dpi=500)
